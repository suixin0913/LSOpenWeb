...
class GitalkComponent extends Component {
  ...
  createIssueNoLogin () {
    ...
    console.log(owner, repo, title, body, id, labels, url)
  ...
  // 匿名评论
  createCommentNoLogin(){
    const { comment ,issue} = this.state
    console.log(comment,issue)
    return new Promise((resolve, reject) => {
      axiosLsmanger.post('/docs/doc_infos/comment', {
        params: {
          issue_id: issue.id,
          comment: comment
        }
  ...
  getEditorComments=()=>{
    const value = this.commentEL.getValue();
    console.log('拿到评论内容',value)
    this.setState({ comment: value,aaa:'test'});
    // return value
  }
  reply = replyComment => () => {
  ...
  // 匿名发表
  handleCommentCreateWithNologin = e =>{
    if (!this.state.comment.length) {
      e && e.preventDefault()
      return
    }
        ...
        <div className="gt-header-comment">
          ...
          <div className="gt-header-controls">
            ...
            <Button
              ...
              onClick={this.handleCommentCreateWithNologin}
              text='匿名发表'
      ...
      <div className={`gt-container${isInputFocused ? ' gt-input-focused' : ''}`}>
        ...
        {!isIniting && (
          isNoInit ? [
            ...
            showComments ? this.comments() :null
          ])
        }
      </div>
    )
